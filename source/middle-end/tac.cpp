/* Implementation file of the three-address code. The TAC instructions
 * are stored in the static class field "tacInstructions."  */
...
int TAC::temporaryCounter = 1;
int TAC::CFLabelCounter = 1;
std::vector<TACInstruction> TAC::tacInstructions;

TACInstruction::TACInstruction(quadOpType op, symTableEntry *arg1Ref, symTableEntry *arg2Ref, symTableEntry *resultRef) {
    this->op = op;
...
void TAC::ResetTACCounters() {
    temporaryCounter = 1;
    CFLabelCounter = 1;
}

std::string TAC::GetNewTemporary() {
    return "_t" + std::to_string(TAC::temporaryCounter++);
}

/* Get new label for control flow statements in three-address code. */
std::string TAC::GetNewCFLabel() {
    return "_L" + std::to_string(TAC::CFLabelCounter++);
}

/* Takes in a source code operator (e.g. "+") and converts it to the corresponding
 * quad enum operator type. */
quadOpType TAC::SourceOptoQuadOp(std::string op, std::string type) {
    // - [] solution for this problem ?
    // if(type != "int" || type != "float") {
    //     cout_str("error : type is not int or float in SourceOptoQuadOp");
    //     return 0;
    // }
...
std::ostream& operator<<(std::ostream& os, quadOpType qType) {
    switch (qType) {
        ...
        case quadOpType::iload : return os << "iload";
        case quadOpType::fload : return os << "fload";
...
void TAC::PrintQuad(TACInstruction quad) {
...
void TAC::PrintQuadArg(quadArg qArg, std::string quadField) {
...
/* Print the quadruples in TAC format. Mainly for debugging.  */
/* -[] clean this up */
void TAC::PrintTACInstructions(std::vector<TACInstruction> instructions) {
    for(TACInstruction &instr : instructions) {
        PrintTACInstruction(instr);
    }
}

void TAC::PrintTACInstruction(TACInstruction instr) {
    std::cout << "    " << GetQuadStr(instr);
}

/* Convert a quad to its string representation. */
std::string TAC::GetQuadStr(TACInstruction instr) {
